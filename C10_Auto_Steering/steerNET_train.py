...
# Intialize the directory where the training image and drivings logs are stored
data_dir = "track"
# Intialize the columns names for loading the drive log files
columns = ["center", "left", "right", "steering", "throttle", "reverse", "speed"]
# Read the training log file with the specific columns names by using pandas library
data = pd.read_csv(os.path.join(data_dir, "driving_log.csv"), names=columns)
pd.set_option('display.max_colwidth', None)
# Display same data using the 'head' method
# print(data.head())


def path_leaf(path):
    # This function provides os.path.functionality on Windows platform
...
def load_img_steering(data_dir, df):
    ...
    for i in range(len(data)):
        ...
        # Left image append
        img_path.append(os.path.join(data_dir, left.strip()))
        steering.append(float(index_data[3] + 0.15))
        # Right image append
        img_path.append(os.path.join(data_dir, right.strip()))
        steering.append(float(index_data[3] - 0.15))
    image_paths = np.asarray(img_path)
...
# The batch-generator method is used for data or an instance of Sequence (keras.utils.Sequence) object 
# in order to avoid duplicate data when using multi-processing
# It provides the tuple objects neither (input, target) or (input, target, sample_weight)
def batch_generator(image_paths, steering_angle, batch_size, is_training):
    while True:
        # Create empty list for batch_img, batch_steering for each batch processing
        batch_img = []
        batch_steering = []

        for i in range(batch_size):
            # size_shaped array of random integers from the appropriate distribution, or a single such random int if size not provided
            random_index = random.randint(0, len(image_paths) - 1)
            # Check if batch-generator is for training or validating
            # If training, it performs random_augmentation and get the img and steering angle
            # If not training, it reads the image_paths with random index to get the image and steering angles
            if is_training:
            ...
            # Perform image processing techniques for converting an img to YUV color, removing blur n resize the image
...
# axs[1].set_title("validating image")
# plt.show()
...
# Train the model on the data batch-by-batch by a Python generator
